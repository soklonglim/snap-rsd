...
const TStr INPUTFILE = "email-Eu-core.txt";
...
int main(int argc, char* argv[]) {
  ...
  TIntFltH NIdStatProb; MLToProbability(NIdStat, NIdStatProb);
  TIntV penaltyBasedKPick; PenaltyBasedKPick(Net, NIdStatProb, penaltyBasedKPick);
...
void PenaltyBasedKPick(const PNEANet& Net, const TIntFltH& NIdStat, TIntV& KSet) {
  ...
  TFlt minPen = TInt::Mx; TInt NId = -1; TStrH pathMemory;
  while (KSet.Len()<K_PICK) {
    printf("KSet.Len: <%d>\n", (int)KSet.Len());
    for (THash<TInt, TFlt>::TIter I = NIdStat.BegI(); I != NIdStat.EndI(); I++) {
      if (KSet.IsIn(I.GetKey())) { continue; }
      TInt attemp = I.GetKey(); KSet.Add(attemp);
      TFlt Pen = MinPenaltyPoint(Net, NIdStat, KSet, pathMemory);
      KSet.DelLast();
...
TFlt MinPenaltyPoint(const PNEANet& Net, const TIntFltH& NIdStat, const TIntV& KSet, TStrH& pathMemory) {
  ...
  for (THash<TInt, TFlt>::TIter I = NIdStat.BegI(); I != NIdStat.EndI(); I++) {
    ...
    for (int i=0; i<KSet.Len(); i++) {
      int src = I.GetKey(); int dst = KSet[i]; int spath;
      char e[128]; sprintf(e, "%d-%d", src, dst);
      if (pathMemory.IsKey(e)) {
        spath = pathMemory.GetDat(e);
      } else {
        spath = TSnap::GetShortPath(Net, src, dst);
        pathMemory.AddDat(e, spath);
      }
