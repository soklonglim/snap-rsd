...
int main(int argc, char* argv[]) {
  ...
  BuildNetworkRandomWeight(InFNm, Net, ReverseNet);
  ...
  if (InSIFNm != INFECTEDFILE) { // load graph from files
    ...
    TRnd Random(IntRnd());
    InfSrcNode = Net->GetRndNId(Random); printf("InfSrcNode: %d, SIMU: %d\n", InfSrcNode, SIMULATIONSZ);
    ...
    if (inf_sz<20) { printf("Infected Graph is too small..."); return (0); } 
    if (inf_sz<200) {
      Q_SZ = 20;
    } else {
  ...
  for (THash<TInt, TFlt>::TIter I = NIdStat.BegI(); I != NIdStat.EndI(); I++) {
    index = index + 1;
    if (I.GetKey() == InfSrcNode) { srcIndex = index - 1; ML = (double)I.GetDat();}
  }
  ...
  TIntV topKPickInDeg; TopKPickInDeg(Sspct, topKPickInDeg); // top k in-degree
  ...
  ShiftMLToPositive(NIdStat);//convert ML to positive first
...
void TopKPickInDeg(const PNGraph& Sspct, TIntV& K_Pick_Result) {
  TIntH DegStat; TIntH Covered; int count = 0;
...
PNGraph SuspectsCandidateGraph(const PNEANet& Net, const TIntFltH& NIdStat) {
  ...
  // printf("Sspct: %d vs NIdStat: %d\n", (int)Sspct->GetNodes(), (int)NIdStat.Len());
  return Sspct;
}
...
/*
void AddWeightNetwork(PNEANet& Net) {  //### add weight to network graph based on in-degree
  for (TNEANet::TNodeI NI = Net->BegNI(); NI < Net->EndNI(); NI++) {
    const int NodeId = NI.GetId();
    const int InDeg = NI.GetInDeg();
    for (int i = 0; i < NI.GetInDeg(); i++) {
      const int NeighborNId = NI.GetInNId(i);
      const int EId = Net->GetEId(NeighborNId, NodeId);
      Net->AddFltAttrDatE(EId, 1.0/InDeg, TSnap::CapAttrName);
    }
  }
  Net->Defrag();
}

void BuildNetworkNoWeight(const TStr& InFNm, PNEANet& Net, PNEANet& ReverseNet) {
  TSsParser Ss(InFNm, ssfWhiteSep, true, true, true);
  Net.Clr(); ReverseNet.Clr();
  Net = TNEANet::New();
  ReverseNet = TNEANet::New();
  ...
  while (Ss.Next()) {
    ...
    Net->AddEdge(SrcNId, DstNId); //### connect the two nodes
    ReverseNet->AddEdge(DstNId, SrcNId);
  }
  Net->Defrag(); //### defragment the net (compact the memory)
  ReverseNet->Defrag();
}
